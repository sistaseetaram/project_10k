...
const DIST_COLORS = ['bg-primary', 'bg-secondary', 'bg-tertiary', 'bg-outline'];

const MILESTONES = [
  { threshold: 1000, title: 'Apprentice Level', desc: '0 - 1,000 Hours. Consistent execution. Core mechanics understood.' },
  { threshold: 2500, title: 'Practitioner Tier', desc: '1,001 - 2,500 Hours. Autonomy developed. Complex problem solving begins.' },
  { threshold: 5000, title: 'Expert Domain', desc: '2,501 - 5,000 Hours. Intuitive grasp of the field. Innovation over repetition.' },
  { threshold: 7500, title: 'Elite Tier', desc: '5,001 - 7,500 Hours. Refined craft. Teaching and mentoring others.' },
  { threshold: 10000, title: 'Mastery Achieved', desc: '10,000 Hours. Unconscious competence. The kinetic light is fully realized.' },
];
...
const Analytics = () => {
  ...
  // current milestone = first not-yet-reached threshold
  const currentIdx = MILESTONES.findIndex((ms) => m.totalHours < ms.threshold);
  const bestPct = m.bestStreak ? Math.min(100, (m.currentStreak / m.bestStreak) * 100) : 0;
    ...
    <div className="max-w-[1200px] mx-auto w-full pb-section">
      ...
      <div className="mb-10">
        ...
        <div className="mt-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-surface-variant pb-2">
          <div className="flex flex-wrap gap-2">
            {['Global Overview', ...m.skillDistribution.slice(0, 3).map((s) => s.name)].map((tab, i) => (
              <button key={tab + i} className={`px-6 py-2.5 text-sm font-medium transition-all border-b-2 active:scale-95 ${i === 0 ? 'border-primary text-primary' : 'border-transparent text-on-surface-variant hover:text-primary hover:bg-primary/5'}`}>
                {tab}
              </button>
            ))}
          </div>
      ...
      {loading ? (
        <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="md:col-span-2 lg:col-span-2 h-64 rounded-xl bg-surface-container-high" />
          <div className="h-64 rounded-xl bg-surface-container-high" />
          <div className="md:col-span-2 lg:col-span-1 h-64 rounded-xl bg-surface-container-high" />
          <div className="md:col-span-2 lg:col-span-2 h-64 rounded-xl bg-surface-container-high" />
        </div>
      ) : sessions.length === 0 ? (
        <div className="bg-white border border-surface-variant rounded-xl p-16 text-center shadow-sm">
          <span className="material-symbols-outlined text-5xl text-on-surface-variant text-slate-300 mb-4">monitoring</span>
          <h2 className="text-xl font-bold text-on-surface mb-2 font-headline">No analytics yet</h2>
          <p className="text-on-surface-variant text-slate-500">Log sessions to see analytics.</p>
        </div>
      ) : (
        <>
          {/* Bento Grid Layout */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="col-span-1 md:col-span-2 lg:col-span-2 bg-white border border-surface-variant rounded-xl p-8 hover:bg-primary-container/5 transition-colors duration-300 flex flex-col justify-between shadow-sm">
              <div>
                ...
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-5xl font-bold text-primary">{fmt(m.totalHours)}</span>
              ...
              <div>
                <div className="flex justify-between text-xs text-on-surface-variant mb-2 text-slate-400">
                  <span>0</span>
                  <span className="font-medium text-primary">{m.globalPct.toFixed(2)}%</span>
                  <span>10k</span>
                </div>
                <div className="w-full bg-surface-container h-4 rounded-full overflow-hidden flex bg-slate-100">
                  <div className="bg-primary h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${Math.max(m.globalPct, 0.5)}%` }}></div>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-1 bg-white border border-surface-variant rounded-xl p-8 hover:bg-primary-container/5 transition-colors duration-300 flex flex-col justify-center items-center text-center shadow-sm">
              ...
              <div className="flex flex-col items-center">
                <span className="text-5xl font-bold text-on-surface tracking-tight">{m.currentStreak}</span>
                <span className="text-lg text-on-surface-variant text-slate-500">Days consecutive</span>
              </div>
              <div className="mt-8 w-full">
                <div className="flex justify-between text-xs text-on-surface-variant mb-2 text-slate-400">
                  <span>Personal Best</span>
                  <span className="font-bold text-on-surface">{m.bestStreak} Days</span>
                </div>
                <div className="w-full bg-surface-container h-2 rounded-full overflow-hidden bg-slate-100">
                  <div className="bg-secondary h-full rounded-full" style={{ width: `${Math.max(bestPct, 0.5)}%` }}></div>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-1 md:col-span-2 lg:col-span-1 bg-white border border-surface-variant rounded-xl p-8 hover:bg-primary-container/5 transition-colors duration-300 shadow-sm">
              ...
              <div className="space-y-6">
                {m.skillDistribution.filter((s) => s.hours > 0).length === 0 ? (
                  <p className="text-sm text-on-surface-variant text-slate-400">No hours logged yet.</p>
                ) : (
                  m.skillDistribution.filter((s) => s.hours > 0).map((skill, i) => (
                    <div key={skill.id}>
                      <div className="flex justify-between text-xs text-on-surface mb-2 font-medium">
                        <span>{skill.name}</span>
                        <span>{fmt(skill.hours)} hrs</span>
                      </div>
                      <div className="w-full bg-surface-container h-3 rounded-full overflow-hidden bg-slate-100">
                        <div className={`${DIST_COLORS[i % DIST_COLORS.length]} h-full rounded-full`} style={{ width: `${Math.max(skill.share, 0.5)}%` }}></div>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="col-span-1 md:col-span-2 lg:col-span-2 bg-white border border-surface-variant rounded-xl p-8 hover:bg-primary-container/5 transition-colors duration-300 shadow-sm">
              ...
              <div className="relative ml-4 md:ml-8 mt-4">
                ...
                <div className="absolute left-[7px] top-2 w-[2px] bg-primary" style={{ height: `${Math.min(100, m.globalPct)}%` }}></div>

                {MILESTONES.map((ms, i) => {
                  const reached = m.totalHours >= ms.threshold;
                  const isCurrent = i === currentIdx;
                  const isFinal = i === MILESTONES.length - 1;
                  const status = reached ? 'done' : isCurrent ? 'current' : isFinal ? 'final' : 'upcoming';
                  return (
                    <div key={ms.threshold} className={`relative flex items-start mb-10 group ${status === 'upcoming' ? 'opacity-60' : status === 'final' ? 'opacity-40' : ''}`}>
                      {status === 'current' ? (
                        ...
                        <div className={`absolute -left-[5px] top-1 w-4 h-4 rounded-full border-2 border-surface-container-lowest z-10 shadow-sm ${status === 'done' ? 'bg-primary' : status === 'final' ? 'bg-white border-outline-variant border-dashed' : 'bg-white border-outline'}`}></div>
                      )}

                      <div className={`ml-8 ${status === 'current' ? 'bg-surface-container-low p-4 rounded-lg border border-primary/20 bg-slate-50' : ''}`}>
                        <div className="flex justify-between items-center mb-1">
                          <h3 className={`text-xl font-bold ${status === 'current' ? 'text-primary' : 'text-on-surface'}`}>{ms.title}</h3>
                          {status === 'current' && <span className="text-xs font-bold text-primary bg-primary-container/20 px-2 py-0.5 rounded uppercase">CURRENT</span>}
                          {status === 'done' && <span className="text-xs font-bold text-primary bg-primary-container/20 px-2 py-0.5 rounded uppercase">REACHED</span>}
                        </div>
                        <p className="text-sm text-on-surface-variant text-slate-500">{ms.desc}</p>
                      </div>
                    </div>
                  );
                })}
